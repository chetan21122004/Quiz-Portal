...
const [test_id, setTest_id] = useState("")
  ...
  const fetchCompletedTests = async (studentId) => {
    ...
    try {
      ...
      console.log('Completed tests data:', completedData);
    } catch (error) {
      console.error('Error in fetchCompletedTests:', error);
    }
  }
  // Data initialization
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (studentData) {
        ...
        // Fetch tests and completed tests
        await fetchTests();
        await fetchCompletedTests(parsedData.id);
      } else {
        await fetchTests();
      }
  ...
  const refreshData = async () => {
    ...
    try {
      await fetchTests();
      if (student.id) {
        await fetchCompletedTests(student.id);
      }
